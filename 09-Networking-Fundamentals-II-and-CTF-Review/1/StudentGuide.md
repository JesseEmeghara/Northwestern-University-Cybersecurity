...
You should be aware of denial of service (DoS) attacks. A DHCP starvation attack is a type of DoS attack that impacts the **availability** aspect of the CIA (confidentiality, integrity, and availability) triad.
...
1. __Distance__: The number of devices or hops used to get the data from the source to the destination.
...
2. **Speed**: The route is determined by the time it takes to move from source to destination. 
    ...
    - Refer to the image in the slides for an illustration of how **OSPF** works.
...
### 12. Decrypting with Aircrack-ng (0:05)
